<?php
/**
 * Template Name: Modèle Bouteilles
 *
 * The template for displaying an immersive, full-screen 3D product slider only for 'Bouteilles'.
 *
 * @package EffePlast
 */
    ...
    <div class="container mx-auto px-4 lg:px-8 relative z-20 mb-12">
        <div class="text-center mb-10">
            ...
            <p class="text-gray-600 font-medium text-lg md:text-xl max-w-2xl mx-auto">
                Explorez notre gamme complète de bouteilles plastiques, conçues pour allier design et fonctionnalité.
            </p>
        </div>
        ...
        <div class="max-w-4xl mx-auto bg-white/80 backdrop-blur-xl border border-gray-100 p-4 md:p-6 rounded-[2rem] shadow-modern flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <!-- Category Pills (Hidden because this page is exclusively for Bouteilles, but we need the data-cat active for JS to fetch correctly) -->
            <div class="flex flex-wrap items-center justify-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0" id="ep-slider-categories" style="display: none;">
                <!-- We hardcode 'bouteilles' as the active category so the JS fetches only these on load -->
                <button class="ep-cat-btn px-4 py-1.5 rounded-full text-xs font-bold tracking-wide transition-all bg-ep-cyan text-white shadow-md shadow-cyan-500/30 border border-ep-cyan ep-active-cat" data-cat="bouteilles">Bouteilles</button>
            </div>
